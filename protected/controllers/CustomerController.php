...
//            'postOnly + delete', // we only allow deletion via POST request
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Customer'])) {
            ...
            $model->gstin_no = $_POST['Customer']['gstin_no'];
            $model->state_code = $_POST['Customer']['state_code'];            
            $model->balance = 0.00;
            if ($model->validate()){
                $model->state= Gststatecodes::model()->findByAttributes(array('state_code'=>$model->state_code))->state_name;
                $model->save();
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Customer'])) {
            $model->gstin_no = $_POST['Customer']['gstin_no'];
            $model->state_code = $_POST['Customer']['state_code'];            
            $model->mobile_no = $_POST['Customer']['mobile_no'];            
            $model->balance = 0.00;
            if ($model->validate()){
                $model->state= Gststatecodes::model()->findByAttributes(array('state_code'=>$model->state_code))->state_name;
                $model->save();
