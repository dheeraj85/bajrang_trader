...
class IndentmasterController extends Controller {
    ...
    public function actionAcceptorder() {
        ...
        $ii = Indentitems::model()->findByPk($indent_itemid);
        $shelf_item = Shelfitems::model()->findByAttributes(array('item_id' => $ii->item_id));
        $shelf_item->total_qty = $shelf_item->total_qty + $_POST['Indentitems']['qty_in_stock'];
    ...
    public function actionIndentupdate() {
        ...
        if (Yii::app()->request->getPost('updateindent')) {
            ...
            $this->redirect(array('indentmaster/review', 'sync_id' => $sync_id));
        }
        if ($_POST['generateorder']) {
            ...
            $this->redirect(array('indentmaster/admin'));
        }
    ...
    public function actionCheckitemexist() {
        $model = Indentitems::model()->findByAttributes(array("item_id" => $_GET['itemid'], "is_indenting_done" => 1));
        if (!empty($model)) {
            $msg = "1";
    ...
    public function actionAdditemindent() {
        ...
        $this->redirect(array('indentmaster/create', 'id' => $_GET['id']));
    }
...
//          exit();
    ...
    public function actionAdmin() {
        ...
        $this->open_class = "gpunit";
        $this->active_class = "admin";
        $model = new Indentmaster('searchGpu');
