  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title alert bg-aqua" id="myModalLabel">Voucher Reconciliation and Expense Nature Assignment</h4>
      </div>
         ...
         <?php
          ...
          ?>
           <div class="form-group">
              <input type="text" placeholder="Security Password" class="form-control" name="auth_password" id="auth_password"/>
...
<script type="text/javascript">
    ...
    $("#authpwd").click(function(){
        ...
        $.ajax({
            url: '<?php echo $this->createUrl('expenseinfo/getsecuritypwd') ?>',
            data: {'valid_data':valid_data,'authpwd':authpwd,'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>'},
            type: 'post',
            success: function(response) {
                if(response=="1"){
                  $('#myModal5').modal('hide');  
                  window.location.href = "<?php echo $this->createUrl('expenseinfo/admin') ?>";
                }else{
