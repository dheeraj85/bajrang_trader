        ...
        <div class="col-lg-12">      
            <div class="box">
                <div class="box-header bg-red">
                    <h3 class="panel-title">Stock Issue Indent</h3>
                </div>
...
<!--                    </div>
                    <div class="row">-->
                        ...
                        <div class='col-md-1' style="margin-top:25px">
                          <?php echo BsHtml::submitButton('Show Availability', array('color' => BsHtml::BUTTON_COLOR_RED,'id'=>'showavail')); ?>
                        </div>
                    </div>
                    <?php $this->endWidget(); ?>
                    <br/>
                      <?php
                        ...
                        if($pid!="" && $psid==""){
                         ...
                         //   echo "select * from purchase_item pi join item_stock ist on ist.item_id=pi.id where pi.item_type in('purchase','processed','resale') and pi.itemname like '%$item_name%'order by pi.id";
                         
                        $alist = Purchaseitem::model()->findAllBySql("select * from purchase_item pi join item_stock ist on ist.item_id=pi.id where pi.item_type in('purchase','processed','resale') and pi.itemname like '%$item_name%'order by pi.id");       
                        }
                        if (!empty($alist)) {
                            ?>
                            <br/>
                            ...
                            $form = $this->beginWidget('bootstrap.widgets.BsActiveForm', array(
                                ...
                                'action' => $this->createUrl('indentmaster/itemevaluate'),
                            ));
                            ?>
                         <div class="table-responsive" style="height:200px;overflow-y:scroll;">
                            ...
                            <input type="submit" id="indentproceed_part1" name="evaluate" value="Proceed &gt;&gt;" class="btn btn-red pull-right" />         
                            <?php $this->endWidget(); ?>
                        <?php }} ?>               
                </div>
            </div>
                 ...
                 <?php
                       if(!empty($pcimodel->id)){    
                        ...
                        if (!empty($ilist)) {
                            ...
                            $form = $this->beginWidget('bootstrap.widgets.BsActiveForm', array(
                                'enableAjaxValidation' => false,
                                'action' => $this->createUrl('indentmaster/indentcomplete'),
                            ));
                            ?>
                            <table class='table table-bordered'>
                                <thead>
                            ...
                            <input type="submit" id="save_indent" name="evaluate" value="Finish Indent &gt;&gt;" class="btn btn-red pull-right" />         
                            <?php $this->endWidget(); ?>
                 <?php }} ?>
                             </div>
                    <br/>
   ...
   function GetSCategory(cid, scid,type) {
    ...
    function checkeditem(id){
       if ($('#checked_item_'+id).is(':checked')) {
          $.getJSON("<?php echo $this->createUrl('indentmaster/checkitemexist'); ?>", {"itemid": id}, function(data) { 
              if(data.msg=="1"){
